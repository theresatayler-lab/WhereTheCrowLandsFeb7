...
def extract_json_from_text(text: str) -> dict:
    ...
    Tries: direct parse, code-fence extraction, then balanced-brace extraction."""
    import re
    ...
    # Balanced brace extraction - find first { and its matching }
    start = stripped.find('{')
    if start != -1:
        depth = 0
        in_string = False
    ...
    raise ValueError(f"Could not extract valid JSON from response: {stripped[:200]}")
...
async def claim_spell_slot(user_id: str, limit: int = 3) -> bool:
    """Atomically claim a spell generation slot. Returns True if slot was claimed.
    Uses findOneAndUpdate with a condition to prevent race conditions."""
    result = await db.users.find_one_and_update(
        {
            'id': user_id,
            'spell_generation_count': {'$lt': limit}
        },
        {
            '$inc': {'spell_generation_count': 1}
        },
        return_document=True
    )
...
# Prompt injection patterns to strip from user inputs before sending to LLMs
_INJECTION_PATTERNS = [
    _re.compile(r'(?i)(ignore|disregard|forget)\s+(all\s+)?(previous|above|prior|earlier)\s+(instructions?|prompts?|rules?|context)', _re.IGNORECASE),
    _re.compile(r'(?i)you\s+are\s+now\s+', _re.IGNORECASE),
    _re.compile(r'(?i)new\s+instructions?\s*:', _re.IGNORECASE),
    _re.compile(r'(?i)system\s*:\s*', _re.IGNORECASE),
    _re.compile(r'(?i)\]\]\s*\n?\s*\[?\[?system', _re.IGNORECASE),
    _re.compile(r'(?i)<<\s*SYS\s*>>', _re.IGNORECASE),
    _re.compile(r'(?i)<\|?(im_start|im_end|system|assistant)\|?>', _re.IGNORECASE),
    _re.compile(r'(?i)(reveal|show|output|print|repeat)\s+(your\s+)?(system\s+)?(prompt|instructions?|rules?)', _re.IGNORECASE),
]

def safe_error_detail(e: Exception, context: str = "operation") -> str:
    """Return a safe error message for HTTP responses without leaking internals."""
    # Log the full error server-side
    logging.error(f"[{context}] {type(e).__name__}: {str(e)}")
...
def sanitize_for_prompt(text: str, max_length: int = 2000) -> str:
    ...
    Strips known injection patterns and enforces length limits."""
    if not text:
        return text
...
async def enhance_single_event(event_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "timeline enhancement"))
...
async def enhance_batch_events(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "batch enhancement"))
...
async def admin_seed_shigg_spells(admin_key: str = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "seeding Shigg spells"))
...
Remember: You are warm, dawn-quiet, British-inflected. Use understatement. Offer tendencies, not predictions. Connect to your literary sources where natural."""
        ...
        safe_situation = sanitize_for_prompt(situation) if situation else ""
        safe_question = sanitize_for_prompt(question) if question else ""
        user_message = f"Seeker's situation: {safe_situation}" if safe_situation else ""
        if safe_question:
            user_message += f"\nTheir question: {safe_question}"
        if not user_message:
        ...
        # Parse JSON
        oracle_data = extract_json_from_text(response_text)
        return {
            "success": True,
            "archetype": {
                "id": "shiggy",
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "bird oracle reading"))
...
async def get_corrie_tarot_reading(request: Request, body: CorrieTarotRequest, user = Depends(get_current_user)):
    ...
    try:
        ...
        # Parse JSON
        reading_data = extract_json_from_text(response_text)
        return {
            "success": True,
            "archetype": {
                "id": "shiggy",
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "Corrie tarot reading"))
...
{{
    ...
    "closing": "Warm Shigg closing (1-2 sentences)"
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "Cobbles oracle reading"))
...
async def suggest_ward(request: Request, body: WardRequest, user = Depends(get_current_user)):
    ...
    try:
        ...
        ward_data = extract_json_from_text(response_text)
        return {
            "success": True,
            "archetype": {
                "id": "kathleen",
        ...
        raise HTTPException(status_code=500, detail=safe_error_detail(e, "ward suggestion"))
...
from prompts import BlocksSpellPipeline, BLOCK_TEMPLATES, CANON_ANCHORS, WORKING_TYPES, run_qa_blocks_validation
...
async def generate_spell_v3_endpoint(request: Request, body: SpellRequestV3, user = Depends(get_current_user)):
    ...
    try:
        ...
        if user:
            ...
            if user_data:
                ...
                if not is_paid:
                    ...
                    spell_slot_claimed = False  # Paid users don't need slots
        ...
        # Increment spell count (skip if already claimed atomically for free users)
        if user and not spell_slot_claimed:
            await increment_spell_count(user['id'])
        elif user and spell_slot_claimed:
            # Slot was pre-claimed atomically; just increment total_spells_generated
            await db.users.update_one(
                {'id': user['id']},
                {'$inc': {'total_spells_generated': 1}}
            )
...
async def get_spell_config_v3():
    ...
    return {
        ...
        'working_types': {
            k: {
                wt_id: {
                    'description': wt_config['description'],
                    'trigger_keywords': wt_config.get('trigger_keywords', [])[:5],
                    'specialty_blocks': wt_config.get('specialty_blocks', [])
                }
...
async def startup_ensure_indexes():
    ...
    try:
        deity_count = await db.deities.count_documents({})
        if deity_count == 0:
            from seed_data import seed_database as _seed_ref
            # Inline seed — avoid running the standalone script's client
            deities = [
                {'id': 'hecate-001', 'name': 'Hecate', 'origin': 'Ancient Greek/Anatolian', 'description': 'Goddess of magic, crossroads, necromancy, and the night. Triple-faced deity associated with the moon and witchcraft.', 'history': 'Hecate was revived in early 20th century occult practice, particularly through the works of Dion Fortune and Aleister Crowley. Her worship was central to the Hermetic Order of the Golden Dawn and influenced modern Wiccan practice.', 'associated_practices': ['Moon rituals', 'Crossroads magic', 'Necromancy', 'Protection spells', 'Divination'], 'image_url': 'https://images.unsplash.com/photo-1711906485337-af335d9810a4?crop=entropy&cs=srgb&fm=jpg&q=85', 'time_period': '1910-1945'},
                {'id': 'morrigan-001', 'name': 'The Morrigan', 'origin': 'Celtic (Irish)', 'description': 'Triple goddess of war, fate, and sovereignty. Shape-shifter associated with crows and prophecy.', 'history': 'The Morrigan experienced renewed interest during the Celtic Revival of the early 20th century. Irish occultists and poets like W.B. Yeats helped revive her worship as a symbol of Irish identity and mystical power.', 'associated_practices': ['Battle magic', 'Prophecy', 'Shape-shifting rituals', 'Sovereignty rites', 'Crow magic'], 'image_url': 'https://images.unsplash.com/photo-1745520470002-391193461501?crop=entropy&cs=srgb&fm=jpg&q=85', 'time_period': '1910-1945'},
                {'id': 'cerridwen-001', 'name': 'Cerridwen', 'origin': 'Celtic (Welsh)', 'description': 'Goddess of transformation, inspiration, and the cauldron of wisdom. Keeper of the potion of knowledge.', 'history': 'Cerridwen became central to Druidic revival practices in the 1920s-1940s, particularly in Wales. Her cauldron symbolism was adopted by early Wiccan practitioners as a feminine divine vessel.', 'associated_practices': ['Potion making', 'Transformation rituals', 'Wisdom seeking', 'Bardic inspiration', 'Cauldron magic'], 'image_url': 'https://images.unsplash.com/photo-1661619669807-784e46af8029?crop=entropy&cs=srgb&fm=jpg&q=85', 'time_period': '1910-1945'},
                {'id': 'arianrhod-001', 'name': 'Arianrhod', 'origin': 'Celtic (Welsh)', 'description': 'Goddess of the silver wheel, fate, and the stars. Guardian of the celestial realm and keeper of destiny.', 'history': 'Arianrhod gained prominence in 1930s occult circles through the works of Dion Fortune, who incorporated her into ritual work focused on lunar magic and feminine mysteries.', 'associated_practices': ['Stellar magic', 'Fate weaving', 'Lunar rituals', 'Wheel of the year', 'Initiation rites'], 'image_url': 'https://images.unsplash.com/photo-1643324896137-f0928e76202a?crop=entropy&cs=srgb&fm=jpg&q=85', 'time_period': '1910-1945'},
            ]
            await db.deities.insert_many(deities)
            logger.info(f"[STARTUP] Seeded {len(deities)} deities")
        
        figures_count = await db.historical_figures.count_documents({})
        if figures_count == 0:
            figures = [
                {'id': 'gardner-001', 'name': 'Gerald Gardner', 'birth_death': '1884-1964', 'bio': 'British civil servant and amateur anthropologist who founded modern Wicca in the 1940s.', 'contributions': 'Founded Wicca, wrote "Witchcraft Today" (1954), established the Book of Shadows.', 'associated_works': ['Witchcraft Today', 'The Meaning of Witchcraft', "High Magic's Aid"], 'image_url': 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?crop=entropy&cs=srgb&fm=jpg&q=85'},
                {'id': 'fortune-001', 'name': 'Dion Fortune', 'birth_death': '1890-1946', 'bio': 'British occultist, ceremonial magician, and novelist. Founded the Society of the Inner Light (1922).', 'contributions': 'Founded Society of the Inner Light, wrote extensively on Western esotericism and Qabalah.', 'associated_works': ['The Mystical Qabalah (1935)', 'Psychic Self-Defence (1930)', 'The Sea Priestess (1938)'], 'image_url': 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?crop=entropy&cs=srgb&fm=jpg&q=85'},
                {'id': 'crowley-001', 'name': 'Aleister Crowley', 'birth_death': '1875-1947', 'bio': 'English occultist, ceremonial magician, and founder of Thelema.', 'contributions': 'Founded Thelema, reformed Golden Dawn ceremonial practices, extensive writings on Qabalah and tarot.', 'associated_works': ['The Book of the Law (1904)', 'Magick in Theory and Practice (1929)', 'The Book of Thoth (1944)'], 'image_url': 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?crop=entropy&cs=srgb&fm=jpg&q=85'},
                {'id': 'yeats-001', 'name': 'W.B. Yeats', 'birth_death': '1865-1939', 'bio': 'Irish poet and occultist, member of the Hermetic Order of the Golden Dawn.', 'contributions': 'Revived Celtic mythology through poetry, member of Golden Dawn, founded Irish Literary Theatre.', 'associated_works': ['The Celtic Twilight', 'A Vision', 'The Wild Swans at Coole'], 'image_url': 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?crop=entropy&cs=srgb&fm=jpg&q=85'},
            ]
            await db.historical_figures.insert_many(figures)
            logger.info(f"[STARTUP] Seeded {len(figures)} historical figures")

        sites_count = await db.sacred_sites.count_documents({})
        if sites_count == 0:
            sites = [
                {'id': 'stonehenge-001', 'name': 'Stonehenge', 'location': 'Wiltshire, England', 'country': 'United Kingdom', 'coordinates': {'lat': 51.1789, 'lng': -1.8262}, 'historical_significance': 'Ancient megalithic monument that became a focal point for Druidic revival in the early 20th century.', 'time_period': '1910-1945', 'image_url': 'https://images.unsplash.com/photo-1588578507406-f90d1df22e8e?crop=entropy&cs=srgb&fm=jpg&q=85'},
                {'id': 'glastonbury-001', 'name': 'Glastonbury Tor', 'location': 'Somerset, England', 'country': 'United Kingdom', 'coordinates': {'lat': 51.1443, 'lng': -2.6988}, 'historical_significance': "Sacred hill associated with Arthurian legend and Celtic mysticism. Connected to Dion Fortune's work.", 'time_period': '1910-1945', 'image_url': 'https://images.unsplash.com/photo-1659016596573-cb626781048a?crop=entropy&cs=srgb&fm=jpg&q=85'},
                {'id': 'tara-001', 'name': 'Hill of Tara', 'location': 'County Meath, Ireland', 'country': 'Ireland', 'coordinates': {'lat': 53.5792, 'lng': -6.6117}, 'historical_significance': 'Ancient seat of Irish High Kings, central to Celtic revival. Site of Morrigan worship and sovereignty rituals.', 'time_period': '1910-1945', 'image_url': 'https://images.unsplash.com/photo-1585932595938-cceb86652be9?crop=entropy&cs=srgb&fm=jpg&q=85'},
            ]
            await db.sacred_sites.insert_many(sites)
            logger.info(f"[STARTUP] Seeded {len(sites)} sacred sites")

        rituals_count = await db.rituals.count_documents({})
        if rituals_count == 0:
            rituals = [
                {'id': 'ritual-001', 'name': 'Drawing Down the Moon', 'description': 'A ritual to invoke the goddess into the high priestess, creating a vessel for divine feminine power.', 'deity_association': 'Hecate, Arianrhod', 'time_period': '1910-1945', 'source': 'Hermetic Order of the Golden Dawn', 'category': 'Invocation'},
                {'id': 'ritual-002', 'name': 'The Lesser Banishing Ritual of the Pentagram', 'description': 'Ceremonial magic ritual for protection and purification.', 'deity_association': None, 'time_period': '1910-1945', 'source': 'Hermetic Order of the Golden Dawn', 'category': 'Protection'},
                {'id': 'ritual-003', 'name': 'Hecate Supper', 'description': 'Offerings left at crossroads on the dark moon for Hecate.', 'deity_association': 'Hecate', 'time_period': '1910-1945', 'source': "Dion Fortune's Society of the Inner Light", 'category': 'Offering'},
                {'id': 'ritual-004', 'name': 'The Great Rite', 'description': 'Symbolic or actual ritual of sacred union representing the joining of god and goddess energies.', 'deity_association': 'Various', 'time_period': '1910-1945', 'source': 'Gardnerian Wicca', 'category': 'Fertility'},
                {'id': 'ritual-005', 'name': 'Cauldron of Cerridwen Meditation', 'description': "Transformative ritual involving visualization of Cerridwen's cauldron for gaining wisdom.", 'deity_association': 'Cerridwen', 'time_period': '1910-1945', 'source': 'Welsh Druidic Revival', 'category': 'Transformation'},
            ]
            await db.rituals.insert_many(rituals)
            logger.info(f"[STARTUP] Seeded {len(rituals)} rituals")
    except Exception as e:
        logger.warning(f"[STARTUP] Seed data error: {e}")
