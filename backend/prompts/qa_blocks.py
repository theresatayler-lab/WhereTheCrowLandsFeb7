...
def _check_template_match(spell: dict, guide_id: str, report: dict):
    """Check that blocks match the guide's template or working type"""
    from .planner_blocks import BLOCK_TEMPLATES, WORKING_TYPES

    blocks = spell.get("blocks", [])
    block_types = [b.get("block_type") for b in blocks]

    # Check if spell has a working_type - if so, validate against that working type's block sequence
    working_type_id = spell.get("working_type") or spell.get("template_id", "").replace(f"{guide_id}_", "")
    guide_working_types = WORKING_TYPES.get(guide_id, {})

    if working_type_id in guide_working_types:
        working_type = guide_working_types[working_type_id]
        required_types = [b["type"] for b in working_type["block_sequence"] if b.get("required", True)]
    else:
    ...
    if missing:
        # Don't double-report universal required blocks
...
def _check_sources_citation(spell: dict, report: dict):
    """Check that spell includes sources with further reading guidance"""
    sources = spell.get("sources", [])

    if not sources or len(sources) < 1:
        report["violations"].append({
            "check": "sources_citation",
            "severity": "HIGH",
            "issue": "Working has no sources cited - must include at least one real source",
            "fix_instruction": "Add at least one real historical, folkloric, or academic source with relevance explanation"
        })
        report["checks_failed"].append("sources_citation: no_sources")
    else:
        # Check that sources have meaningful content
        valid_sources = [s for s in sources if s.get("author") or s.get("work")]
        if len(valid_sources) < 1:
            report["violations"].append({
                "check": "sources_citation",
                "severity": "HIGH",
                "issue": "Sources are present but lack author/work details",
                "fix_instruction": "Each source must include author and work title"
            })
            report["checks_failed"].append("sources_citation: incomplete_sources")
        else:
            report["checks_passed"].append("sources_citation")
